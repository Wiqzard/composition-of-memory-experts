...
class Pips(nn.Module):
    ...
    def forward(
        ...
        if feat_init is not None:
            ...
            feat1 = bilinear_sample2d(fmaps[:, 0], coords[:, 0, :, 0], coords[:, 0, :, 1]).permute(
                0, 2, 1
            )  # B,N,C
        ...
        for itr in range(iters):
            ...
            if itr >= 1:
                ...
                feats2_ = bilinear_sample2d(fmaps2_, coords2_[:, :, 0], coords2_[:, :, 1]).permute(
                    0, 2, 1
                )  # B*S, N, C
                feats2 = feats2_.reshape(B, S, N, self.latent_dim)
                feats4_ = bilinear_sample2d(fmaps4_, coords4_[:, :, 0], coords4_[:, :, 1]).permute(
                    0, 2, 1
                )  # B*S, N, C
