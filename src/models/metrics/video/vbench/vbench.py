...
from src.models.metrics.video.shared_registry import SharedVideoMetricModelRegistry
from src.models.metrics.video.utils import videos_as_images
...
class VBench(Metric):
        ...
        self.dimensions = torch.nn.ModuleDict(
            {
                ...
                VBenchDimensionType.BACKGROUND_CONSISTENCY: BackgroundConsistency(registry),
                ...
                VBenchDimensionType.MOTION_SMOOTHNESS: MotionSmoothness(registry, resolution),
            }
        )
    ...
    def update(self, videos: Tensor) -> None:
        ...
        for dimension in self.ALL_DIMENSIONS:
            getattr(self, f"{dimension}_scores").append(self.dimensions[dimension](videos))
    ...
    def compute(self) -> Dict[str, Tensor]:
        ...
        normalized_scores = {
            dimension: dimension.normalize_score(score) for dimension, score in scores.items()
        }
        final_score = torch.stack(
            [dimension.weight_score(score) for dimension, score in normalized_scores.items()]
