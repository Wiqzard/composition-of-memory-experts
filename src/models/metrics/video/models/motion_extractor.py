...
class MotionExtractor(nn.Module):
    ...
    def _get_coords(self) -> Tensor:
        ...
        grid_y = 8 + grid_y.reshape(-1).float() / (sqrt_num_points - 1) * (self.resolution - 16)
        grid_x = 8 + grid_x.reshape(-1).float() / (sqrt_num_points - 1) * (self.resolution - 16)
        coords = torch.stack([grid_x, grid_y], dim=-1)  # N, 2
        return repeat(coords, "n d -> b s n d", b=1, s=self.segment_length)  # 1, S, N, 2
    ...
    def _calc_velocity(trajectories: Tensor) -> Tensor:
        ...
        velocity = torch.cat([torch.zeros_like(velocity[:, 0:1]), velocity], dim=1)  # B,S,N,2
        return velocity
