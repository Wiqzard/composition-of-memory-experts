...
class ContinuousDiffusion(nn.Module):
    ...
    def ddim_sample_step(
        ...
        **kwargs,
        ...
        # ---------------------------
        # Stage 1: student estimate at t
        # ---------------------------
        # adapter = student θ ; teacher = self.model (ψ)
        student_pred, *_ = self.model_predictions(
            x=x,
            k=clipped_curr_noise_level,
            external_cond=external_cond,
            external_cond_mask=external_cond_mask,
            adapter=adapter,  # student
            adapter_only=True,  # crucial: ONLY student
            **kwargs,
        )
        x_start_student = student_pred.pred_x_start  # \hat x_0^θ(t)

        # ---------------------------
        # Stage 2: teacher revision (first 'teacher_k' steps)

        # ---------------------------
        teacher_lambda = 0.02  # cfg_scale
        teacher_k = 1
        # step_idx = k
        use_teacher = True  # (
        # (teacher_lambda > 0.0) and (teacher_k > 0)
        # and (step_idx is not None) and (step_idx < teacher_k)
        # )

        if use_teacher:
            #### CHECK IF THIS SHOULD NOT BE THE PREVIOUS NOISE LEVEL
            # s = t - Δt  (i.e., the next noise level)
            s = torch.clamp(next_noise_level, min=0)

            # Re-noise student x0 at level s
            alpha_s = self.add_shape_channels(self.alphas_cumprod[s])
            eps_s = torch.randn_like(x)
            eps_s = torch.clamp(eps_s, -self.clip_noise, self.clip_noise)
            x_s = alpha_s.sqrt() * x_start_student + (1.0 - alpha_s).sqrt() * eps_s

            # Teacher prediction at s (no adapter)
            teacher_pred, *_ = self.model_predictions(
                x=x_s,
                k=s,
                external_cond=external_cond,
                external_cond_mask=external_cond_mask,
                adapter=None,  # teacher only
                adapter_only=False,
                **kwargs,
            )
            x_start_teacher = teacher_pred.pred_x_start  # \hat x_0^ψ(s)

            # Mix student and teacher x0
            x_start = (1.0 - teacher_lambda) * x_start_student + teacher_lambda * x_start_teacher
        else:
            x_start = x_start_student

        # From x and \hat x_0 (at time t), compute \hat ε_t and proceed with standard DDIM update
        pred_noise = torch.where(
            (1.0 - alpha).sqrt() > 0,
            (x - alpha.sqrt() * x_start) / (1.0 - alpha).sqrt(),
            torch.zeros_like(x),
        )
        aux_output = None
