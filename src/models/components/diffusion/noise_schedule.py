...
def make_beta_schedule(
    ...
    **kwargs,
    ...
    return torch.clip(betas, clip_min, clip_max)
...
def cosine_simple_diffusion_schedule(
    ...
    if shifted != 1.0:
        shifted_logsnr = logsnr + 2 * torch.log(torch.tensor(shifted, dtype=torch.float64))
        if interpolated:
...
def beta_linear_schedule(timesteps: int, start: float = 0.0001, end: float = 0.02) -> torch.Tensor:
    """
    linear schedule
...
def shift_beta_schedule(alphas_cumprod: torch.Tensor, shift: float):
    ...
    return (snr_scale * alphas_cumprod) / (snr_scale * alphas_cumprod + 1 - alphas_cumprod)
