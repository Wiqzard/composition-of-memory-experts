...
from src.models.components.modules.embeddings import TimestepEmbedding, Timesteps
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, emb: Optional[torch.Tensor] = None):
        ...
        if self.emb_layers is not None:
            assert emb is not None, "Noise level embedding is required for this ResnetBlock"
...
class LinearAttention(nn.Module):
    def __init__(
        ...
        if rotary_emb is not None:
            raise NotImplementedError("Rotary embeddings not implemented for linear attention")
    ...
    def forward(
        ...
        if is_causal:
            raise NotImplementedError("Causal masking not implemented for linear attention")
        if attn_mask is not None:
            raise NotImplementedError("Attention masking not implemented for linear attention")
...
class TemporalAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        ...
        if self.time_pos_embedding is not None:
            ...
            time_emb = self.time_pos_embedding(torch.arange(num_frames, device=x.device))
            x = x + time_emb
...
UnetSpatialAttentionBlock = get_einops_wrapped_module(AttentionBlock, "b c t h w", "(b t) (h w) c")
...
class UnetTemporalAttentionBlock(_UnetTemporalAttentionBlock):
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        if attn_mask is not None:
            attn_mask = repeat(attn_mask, "b t1 t2 -> (b h w) t1 t2", h=x.shape[-2], w=x.shape[-1])
