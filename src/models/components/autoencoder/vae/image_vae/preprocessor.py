...
class ImageVAEPreprocessor(BaseLightningTrainer):
    ...
    def test_step(self, batch, batch_idx) -> STEP_OUTPUT:
        raise NotImplementedError("Testing not implemented for VAEVideo. Only used for validation")
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0) -> STEP_OUTPUT:
        ...
        if batch_idx % self.log_every_n_batch == 0 and self.logger:
            ...
            log_video(
                ...
                captions=[
                    f"{p.parent.parent.name}/{p.parent.name}/{p.stem}" for p in latent_paths
                ],
            )
