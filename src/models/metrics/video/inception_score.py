...
class InceptionScore(Metric):
    ...
    def __init__(self, num_classes: int = 400, **kwargs: Any):
        ...
        self.add_state("prob_sum", torch.zeros(num_classes).double(), dist_reduce_fx="sum")
        self.add_state("num_samples", torch.tensor(0).long(), dist_reduce_fx="sum")
        self.add_state("neg_entropy_sum", torch.tensor(0.0).double(), dist_reduce_fx="sum")
    ...
    def compute(self) -> Tensor:
        ...
        kl = self.neg_entropy_sum / self.num_samples - (mean_prob * mean_prob.log()).sum()
        # calculate IS
