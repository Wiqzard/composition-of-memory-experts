...
def quadratic_beta_schedule(timesteps):
    """
    quadratic schedule, proposed in SD
    """
    beta_start = math.sqrt(0.00085)
    beta_end = math.sqrt(0.012)
    sqrt_betas = torch.linspace(beta_start, beta_end, timesteps, dtype=torch.float64)
    return sqrt_betas**2


def cosine_noise_schedule(T, beta_max=0.1, s=0.008, device=None, dtype=torch.float32):
    """
    Cosine noise schedule from “Elucidating the Design Space of Diffusion-Based Generative Models”
    (Nichol & Dhariwal, 2021): https://arxiv.org/abs/2102.09672

    Args:
        T (int): total number of timesteps.
        beta_max (float): maximum β_t (clips the schedule).
        s (float): small offset to adjust the curve.
        device (torch.device or str, optional): where to create the tensors.
        dtype (torch.dtype, optional): tensor dtype (default: torch.float32).

    Returns:
        beta_t (torch.Tensor): shape (T+1,), β values at each t.
        alpha_t (torch.Tensor): shape (T+1,), cumulative product of (1–β).
    """
    t = torch.arange(0, T + 1, device=device, dtype=dtype)
    freq = ((t / T) + s) / (4 * (1 + s))
    alphas = torch.cos(2 * math.pi * freq) ** 2
    # normalize so alpha_0 = 1
    alphas = alphas / alphas[0]
    # β_t = 1 – α_t / α_{t-1}, with β₀ = 0
    betas = torch.zeros_like(alphas)
    betas[1:] = 1 - alphas[1:] / alphas[:-1]
    betas = torch.clamp(betas, 0.0, beta_max)
    return betas, alphas
...
class DiscreteDiffusion(nn.Module):
    ...
    def _build_buffer(self):
        ...
        # betas = torch.linspace(0.0001, 0.02, self.timesteps, dtype=torch.float32)
        ...
        **kwargs,
        ...
        if self.objective == "pred_noise":
            ...
            if pred_noise is not None and adapter_output is not None:
                # pred_noise[:, 1:] = 0.4 * pred_noise[:, 1:] + pred_adapter_noise[:, 1:]
    ...
    def forward(
        ...
        **kwargs,
        ...
        model_pred, _ = self.model_predictions(
            ...
            external_cond=external_cond,
            adapter=adapter,
            adapter_only=(adapter is not None),
            # only_model=(adapter is None),
            **kwargs,
        )
        ...
        **kwargs,
        ...
        if guidance_fn is not None:
            ...
            model_pred, *aux_output = self.model_predictions(
                ...
                cfg_scale=cfg_scale,
                adapter=adapter,
                adapter_only=True,  # False, #True, #False, # True
                **kwargs,
            )
            x_start = model_pred.pred_x_start
        ...
        if n_ula > 0 and next_noise_level[0, -1].item() > 100:
            for _ in range(n_ula):
                ...
                x_pred = (
                    x_pred - beta_t / (2 * (1 - alpha_next).sqrt()) * eps + beta_t.sqrt() * eta
                )
