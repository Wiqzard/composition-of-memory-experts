...
def estimate_latent_stats(
    ...
    latent_paths = [p for p in data_dir.glob("**/*.pt") if not p.name.endswith("_cond.pt")]
    random.shuffle(latent_paths)
    means, stds = [], []

    for i in range(repeat):
        latents = None
        for idx in tqdm(range(batch_size * i, batch_size * (i + 1)), desc=f"Batch {i + 1}"):
    ...
    if channel_wise:
        overall_mean = [sum(m[i] for m in means) / len(means) for i in range(len(means[0]))]
