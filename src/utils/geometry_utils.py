...
class Ray:
    ...
    def _nerf_pos_encoding(x: torch.Tensor, freq: int) -> torch.Tensor:
        scale = 2 ** torch.linspace(0, freq - 1, freq, device=x.device, dtype=x.dtype) * math.pi
...
class CameraPose:
        ...
        for b in range(mask.shape[0]):
            ...
            for left_t, right_t in zip(valid_ts[:-1], valid_ts[1:]):
                ...
                T[b, left_t : right_t + 1] = torch.lerp(
                    T[b, left_t],
                    T[b, right_t],
                    torch.linspace(0, 1, right_t - left_t + 1, device=T.device).unsqueeze(-1),
                )
    ...
    def extrinsics(self, flatten: bool = False) -> torch.Tensor:
        ...
        """
        extrinsics = torch.cat([self._R, rearrange(self._T, "b t i -> b t i 1")], dim=-1)
    ...
    def rays(self, resolution: int) -> Ray:
        ...
        fx, fy, px, py = rearrange(self._K * resolution, "b t i -> b t i 1").chunk(4, dim=-2)
        x = (coord_w - px) / fx
        y = (coord_h - py) / fy
        z = torch.ones_like(x)
        ...
        origin = repeat(origin, "b t i -> b t h w i", h=resolution, w=resolution).clone()
