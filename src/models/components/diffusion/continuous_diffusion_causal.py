...
class ContinuousDiffusion(nn.Module):
        ...
        **kwargs,
        ...
        if not adapter_only:
            model_output = self.model(
                ...
                x[:, 1:],
                self.precond_scale * self.logsnr[k][:, 1:],
                external_cond[:, 1:],
                x_clean=x[:, :-1],
            )
        ...
        if self.objective == "pred_noise":
            ...
            pred_noise = None
            if model_output is not None:
                pred_noise = self.predict_noise_from_v(x[:, 1:], k[:, 1:], model_output)
                pred_noise = torch.cat([torch.zeros_like(x[:, :1]), pred_noise], dim=1)
    ...
    def forward(self, x, external_cond, k, adapter=None, patch_wise_noie_level=False, **kwargs):
        ...
        v_pred = model(
            x_t[:, 1:],
            self.precond_scale * logsnr[:, 1:],
            external_cond[:, 1:],
            x_clean=x[:, :-1],
            **kwargs,
        )

        noise_pred = alpha_t[:, 1:] * v_pred + sigma_t[:, 1:] * x_t[:, 1:]
        x_pred = alpha_t[:, 1:] * x_t[:, 1:] - sigma_t[:, 1:] * v_pred
        x_pred = torch.cat([x[:, :1], x_pred], dim=1)

        loss = F.mse_loss(noise_pred, noise[:, 1:], reduction="none")
        bias = self.sigmoid_bias
        loss_weight = torch.sigmoid(bias - logsnr)
        loss_weight = self.add_shape_channels(loss_weight)
        loss = loss * loss_weight[:, 1:]
