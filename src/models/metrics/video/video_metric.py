...
class VideoMetric(nn.Module):
    ...
    def _update(self, preds: Tensor, target: Tensor, context_mask: Optional[Tensor] = None):
        """
        Note:
        ...
        if context_mask is None:
            context_mask = torch.zeros(preds.shape[1], device=preds.device, dtype=torch.bool)

        # replace all NaNs with 0 / clamp to [0, 1] / convert to float32
        preds, target = map(
            lambda x: torch.clamp(torch.nan_to_num(x, nan=0.0), 0.0, 1.0).to(torch.float32),
            (preds, target),
        )
        ...
        if i3d_dependent_metrics:
            ...
            for metric_type, module in self._filtered_items(self.I3D_DEPENDENT_METRICS):
                if metric_type == VideoMetricType.FVD:
                    ...
                    module.update(
                        fake_features if metric_type == VideoMetricType.IS else real_features
                    )
    ...
    def log(self, prefix: str):
        ...
        for metric_type, module in self._filtered_items(self.VBENCH_METRICS):
            ...
            dict_metrics.update(
                {f"{prefix}/{metric_type}/{key}": value for key, value in output.items()}
            )
