...
from utils.ckpt_utils import download_pretrained as hf_to_local_path
from utils.ckpt_utils import is_hf_path, is_wandb_run_path, wandb_to_local_path
from utils.logging_utils import get_validation_metrics_for_videos, log_video
...
class ImageVAETrainer(pl.LightningModule):
    def __init__(
        ...
        self.quant_conv = torch.nn.Conv2d(2 * ddconfig["z_channels"], 2 * self.embed_dim, 1)
    ...
    def on_after_batch_transfer(self, batch: tuple, dataloader_idx: int = 0) -> torch.Tensor:
        x = batch["videos"]
    ...
    def training_step(self, batch, batch_idx):
        ...
        if False:
            # do linear downsampleing randomly to 2 to 4 times smaller of batch
            batch_small = batch
            if self.global_step % 2 == 0:
                downsample_factor = 2 if self.global_step % 4 == 0 else 4
                batch_small = torch.nn.functional.interpolate(
                    batch, scale_factor=1.0 / downsample_factor, mode="bilinear"
                )
                reconstructions_small, posterior_small = self(batch_small)
        ...
        if self.global_step < self.warmup_steps:
            ...
            should_warmup = True
            lr_scale = (
                float(self.global_step - self.cfg.lossconfig.disc_start + 1) / self.warmup_steps
            )
        ...
        if False:
            aeloss_small, log_dict_ae_small = self.loss(
                batch_small,
                reconstructions_small,
                posterior_small,
                0,
                self.global_step,
                last_layer=self.get_last_layer(),
            ...
            aeloss = aeloss + aeloss_small
            log_dict_ae.update(log_dict_ae_small)
    ...
    def _rearrange_and_unnormalize(self, batch: torch.Tensor, batch_size: int) -> torch.Tensor:
