...
class StochasticUnknownTimesteps(Timesteps):
    def __init__(
        ...
        self.unknown_token = nn.Parameter(torch.randn(1, num_channels)) if p > 0.0 else None
        self.p = p
...
class StochasticTimeEmbedding(nn.Module):
    ...
    def forward(self, timesteps: torch.Tensor, mask: Optional[torch.Tensor] = None):
        return self.embedding(
            self.timesteps(timesteps) if self.use_fourier else self.timesteps(timesteps, mask)
        )
