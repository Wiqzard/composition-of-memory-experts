...
from algorithms.common.metrics.video import SharedVideoMetricModelRegistry, VideoMetric
...
from utils.torch_utils import bernoulli_tensor, freeze_model
...
class DFoTVideo(BasePytorchAlgo):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.tasks = [
            task for task in ["prediction", "interpolation"] if getattr(cfg.tasks, task).enabled
        ]
    ...
    def _build_model(self):
        # 1. Diffusion model
        diffusion_cls = (
            ContinuousDiffusion if self.cfg.diffusion.is_continuous else DiscreteDiffusion
        )
    ...
    def validation_step(self, batch, batch_idx, namespace="validation") -> STEP_OUTPUT:
        ...
        if not (self.trainer.sanity_checking and not self.cfg.logging.sanity_generation):
    ...
    def on_validation_epoch_start(self) -> None:
        if self.cfg.logging.deterministic is not None:
            self.generator = torch.Generator(device=self.device).manual_seed(
                self.global_rank + self.trainer.world_size * self.cfg.logging.deterministic
            )
    ...
    def _eval_denoising(self, batch, batch_idx, namespace="training") -> None:
        ...
        if not (
            is_rank_zero and self.logger and self.num_logged_videos < self.logging.max_num_videos
        ):
            return
    ...
    def _predict_videos(self, xs: Tensor, conditions: Optional[Tensor] = None) -> Tensor:
        """
    ...
    def _interpolate_videos(
        ...
        pbar = tqdm(
            total=sum(
                [
                    ((len(frames) + max_batch_size - 1) // max_batch_size if max_batch_size else 1)
                    for frames in plan
                ]
            )
        ...
        for current_plan in plan:
            ...
            for frames in current_plan:
                ...
                current_context_mask.append(self._pad_to_max_tokens(context_mask[:, frames]))
                if conditions is not None:
                    current_conditions.append(self._pad_to_max_tokens(conditions[:, frames]))
    ...
    def _update_metrics(self, all_videos: Dict[str, Tensor]) -> None:
        ...
        if (
            ...
            all_videos = {k: v[:, : self.logging.n_metrics_frames] for k, v in all_videos.items()}
    ...
    def _get_training_noise_levels(
        ...
        if self.cfg.variable_context.enabled:
            assert not self.cfg.fixed_context.enabled, "Cannot use both fixed and variable context"
            ...
            context_indices = self.cfg.fixed_context.indices or list(range(self.n_context_tokens))
        ...
        if context_mask is not None:
            ...
            if not self.cfg.diffusion.is_continuous:
                context_noise_levels = context_noise_levels.long() * (self.timesteps - 1)
    ...
    def _generate_scheduling_matrix(
        ...
        match self.cfg.scheduling_matrix:
            case "full_sequence":
                scheduling_matrix = np.arange(self.sampling_timesteps, -1, -1)[:, None].repeat(
                    horizon, axis=1
                )
            case "autoregressive":
        ...
        scheduling_matrix = F.pad(scheduling_matrix, (0, padding, 0, 0), value=self.timesteps - 1)
            ...
            with history_guidance(context_mask) as history_guidance_manager:
                ...
                if reconstruction_guidance > 0:
                    ...
                    def composed_guidance_fn(
                        ...
                        loss = (
                            F.mse_loss(pred_x0, context, reduction="none") * alpha_cumprod.sqrt()
                        )
    ...
    def check_video_vae_compatibility(self, cfg: DictConfig):
        ...
        """
        assert cfg.latent.type == "online", "Latents must be processed online when using VideoVAE."
        assert (
