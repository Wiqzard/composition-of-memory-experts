...
class LPIPSWithDiscriminator(nn.Module):
    ...
    def forward(
        ...
        if optimizer_idx == 0:
            ...
            log = {
                f"{split}/total_loss": loss.clone().detach().mean().item(),
                # pylint: disable=not-callable
                f"{split}/logvar": self.logvar.detach().item(),
                ...
                f"{split}/g_loss": g_loss.detach().mean().item(),
            }
...
class LPIPSWithDiscriminator3D(nn.Module):
    ...
    def calculate_adaptive_weight(self, nll_loss, g_adversarial_loss, last_layer):
        nll_grads = torch.autograd.grad(nll_loss, last_layer, retain_graph=True)[0]
        g_grads = torch.autograd.grad(g_adversarial_loss, last_layer, retain_graph=True)[0]
        d_weight = torch.norm(nll_grads) / (torch.norm(g_grads) + 1e-4)
        d_weight = torch.clamp(d_weight, 0.0, 1e4).detach()
    ...
    def forward(
        ...
        if optimizer_idx == 0:
            ...
            nll_loss = (rec_loss + self.perceptual_weight * p_loss).sum() / rec_loss.shape[0]
            kl_loss = posteriors.kl()
            kl_loss = kl_loss.mean()  # this ensures that the kl_loss is per batch & frame
            ...
            g_adversarial_loss_weight = (
                adopt_weight(self.disc_factor, global_step, threshold=self.disc_iter_start)
                * d_weight
            )
