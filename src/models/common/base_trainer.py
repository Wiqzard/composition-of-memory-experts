...
class BaseLightningTrainer(pl.LightningModule, ABC):
    """
    ...
    def gather_data(self, data: Union[torch.Tensor, Dict, List, Tuple], batch_dim: int = 0):
        """
        ...
        rearrange_fn = (
            lambda x: x.permute(
                list(range(1, batch_dim + 1)) + [0] + list(range(batch_dim + 1, x.dim()))
            )
