...
def extend_x_dim(x: torch.Tensor, x_shape) -> torch.Tensor:
    """
    Extend the 2D [batch, tokens] mask to match the shape of your x_shape (i.e., [batch, tokens, c, h, w]).
    """
    return rearrange(
        x,
        "... -> ... " + "1 " * len(x_shape),  # e.g. " -> ... 1 1 1" if x_shape=[3,64,64]
    )
...
class ContinuousDiffusion(nn.Module):
    ...
    def model_predictions(
        ...
        adapter_stm=None,
        full_xs_pred=None,
        clipped_curr_noise_level_stm=None,
        full_conditions=None,
        full_context_mask=None,
        stm_only=False,
        full_xs_pred_init_noise=None,  # full_xs_pred_init_noise
        orig_noise=None,
        context_mask=None,
        mem_conditions=None,
        adapter_model_uncond=None,
        **kwargs,
    ):
        """
        ...
        if True:
            t = context_mask.sum().item()
            xs_pred_uncond = torch.cat([orig_noise[:, :t], x[:, t:]], dim=1)
            uncond_clipped_curr_noise_level = torch.where(context_mask == 1, self.timesteps - 1, k)
            model_output_uncond = self.model(
                xs_pred_uncond,
                self.precond_scale * self.logsnr[uncond_clipped_curr_noise_level],
                external_cond,
                external_cond_mask,
                **kwargs,
            )
            pred_noise_uncond = self.predict_noise_from_v(
                xs_pred_uncond, uncond_clipped_curr_noise_level, model_output_uncond
            )

        adapter_output = None
        pred_adapter_noise = None
        pred_adapter_ltm_noise_uncond = None
        pred_adapter_stm_noise = None
        adapter_stm_output = None

        if adapter is not None:  # and not only_model:
            adapter_output = adapter(
                x,
                self.precond_scale * self.logsnr[k],
                external_cond if mem_conditions is None else mem_conditions,
                external_cond_mask,
                **kwargs,
            )
            pred_adapter_noise = torch.clamp(adapter_output, -self.clip_noise, self.clip_noise)

        if adapter_model_uncond is not None:
            adapter_ltm_output_uncond = adapter_model_uncond(
                x,
                self.precond_scale * self.logsnr[k],
                external_cond if mem_conditions is None else mem_conditions,
                external_cond_mask,
            ...
            pred_adapter_ltm_noise_uncond = self.predict_noise_from_v(
                x, k, adapter_ltm_output_uncond
            )

        if adapter_stm is not None:  # and not only_model:

            adapter_stm_output = adapter_stm(
                full_xs_pred,
                self.precond_scale * self.logsnr[clipped_curr_noise_level_stm],
                full_conditions,
                None,  # full_context_mask,
                **kwargs,
            )

        if True:
            t = full_xs_pred_init_noise.shape[1]
            full_xs_pred_uncond = torch.cat([full_xs_pred_init_noise, full_xs_pred[:, t:]], dim=1)
            # full_xs_pred_uncond = torch.where(extend_x_dim(full_context_mask, full_xs_pred.shape[2:]) == 1, full_xs_pred, torch.randn_like(full_xs_pred))
            uncond_clipped_curr_noise_level_stm = torch.where(
                full_context_mask == 1, self.timesteps - 1, clipped_curr_noise_level_stm
            )
            adapter_stm_output_uncond = adapter_stm(
                full_xs_pred_uncond,
                self.precond_scale * self.logsnr[uncond_clipped_curr_noise_level_stm],
                full_conditions,
                None,  # full_context_mask,
                **kwargs,
            )
            pred_adapter_stm_noise_uncond = self.predict_noise_from_v(
                full_xs_pred_uncond, uncond_clipped_curr_noise_level_stm, adapter_stm_output_uncond
            )[:, -self.max_tokens :]

        if adapter_output is not None:
            pred_adapter_noise = self.predict_noise_from_v(x, k, adapter_output)
        if adapter_stm_output is not None:
            pred_adapter_stm_noise = self.predict_noise_from_v(
                full_xs_pred, clipped_curr_noise_level_stm, adapter_stm_output
            )[:, -self.max_tokens :]
            if pred_adapter_noise is not None and not stm_only:
                # pred_adapter_noise = 0.5 * pred_adapter_noise + 0.5 * pred_adapter_stm_noise
                # pred_adapter_noise = 1.0 * pred_adapter_noise + 1.0 * pred_adapter_stm_noise
                # pred_adapter_noise = 1/3 * pred_adapter_noise + 1/3 * pred_adapter_stm_noise
                a = 0
            elif stm_only:
                pred_adapter_noise = pred_adapter_stm_noise
        ...
        if pred_noise is not None and adapter_output is not None:
            ...
            #  + cfg_scale * (pred_adapter_stm_noise - pred_noise)
            # )
            # pred_noise = (
            #    pred_noise
            #    + cfg_scale * (pred_adapter_noise - pred_noise)
            #   + 0.2 * pred_adapter_stm_noise
            # )

            ##################### CUSTOM ##################
            alpha_1 = 2  # 3 #2.5
            if pred_adapter_ltm_noise_uncond is not None:
                ltm_score = (1 - alpha_1) * pred_adapter_ltm_noise_uncond + (alpha_1) * (
                    pred_adapter_noise  # + 0.1 * pred_noise
                )
            else:
                ltm_score = (1 - alpha_1) * pred_noise_uncond + (alpha_1) * (
                    pred_adapter_noise  # + 0.1 * pred_noise
                )
            # ltm_score = (1 - alpha_1) * pred_noise + (alpha_1) * pred_adapter_noise
            alpha_2 = 2
            stm_score = (1 - alpha_2) * pred_adapter_stm_noise_uncond + (alpha_2) * (
                pred_adapter_stm_noise  # + 0.1 * pred_noise
                ## + 0.1 !!!!!!
                ## + 0.1 !!!!!!
                ## + 0.1 !!!!!!
            )
            ###
            # stm_score = (1 - alpha_2) * pred_adapter_stm_noise_uncond + (alpha_2) * (
            #    pred_adapter_stm_noise
            # )
            ###
            ### here is the prior missing
            # pred_noise = stm_score + ltm_score
            pred_noise = stm_score + ltm_score
            # pred_noise = ltm_score
            # naive adding
            # pred_noise =  stmpred_adapter_stm_noise + pred_noise #+ pred_adapter_noise

            # pred_noise = 0.5 * ltm_score + 0.5 * stm_score
            # pred_noise =  stm_score
            # pred_adapter_stm_noise: ok
            ################################################

            # pred_noise = pred_noise + 1.25 * (
            #    pred_adapter_noise + pred_adapter_stm_noise - pred_noise
            # )
            # pred_noise =  pred_adapter_noise + pred_adapter_stm_noise

            # pred_noise = pred_noise + pred_adapter_noise
            # pred_noise = pred_noise + pred_adapter_noise
            ...
            # pred_noise = 0.5 * pred_adapter_noise + 0.5 * pred_noise
    ...
    def forward(
        self, x, external_cond, k, adapter=None, adapter_stm_model=None, combine=False, **kwargs
    ):
        ...
        if combine:
            v_pred_adapter = self.adapter(
                x_t,
                self.precond_scale * logsnr,
                external_cond,
                **kwargs,
            )
            noise_pred_adapter = alpha_t * v_pred_adapter + sigma_t * x_t
            noise_pred = noise_pred + noise_pred_adapter
        ...
        **kwargs,
        ...
        if self.is_ddim_sampling:
            return self.ddim_sample_step(
                ...
                stm_only=stm_only,
                full_xs_pred_init_noise=full_xs_pred_init_noise,
                orig_noise=orig_noise,
                context_mask=context_mask,
                mem_conditions=mem_conditions,
                adapter_model_uncond=adapter_model_uncond,
                **kwargs,
            )
    ...
    def _ula_step(
        ...
        **kwargs,
        ...
        factor_1 = extract(self.betas, k, x_k.shape) / extract(
            self.sqrt_one_minus_alphas_cumprod, k, x_k.shape
        )
    ...
    def ddim_sample_step(
        ...
        full_conditions=None,
        full_context_mask=None,
        from_noise_levels_stm=None,
        to_noise_levels_stm=None,
        stm_only=False,
        full_xs_pred_init_noise=None,  # full_xs_pred_init_noise
        orig_noise=None,
        context_mask=None,
        ...
        **kwargs,
        ...
        clipped_curr_noise_level = torch.clamp(curr_noise_level, min=0)
        if from_noise_levels_stm is not None:
            clipped_curr_noise_level_stm = torch.clamp(from_noise_levels_stm, min=0)
        ...
        model_pred, *aux_output = self.model_predictions(
            ...
            adapter_only=False,
            adapter_stm=adapter_stm,
            clipped_curr_noise_level_stm=(
                clipped_curr_noise_level_stm if from_noise_levels_stm is not None else None
            ),
            full_xs_pred=full_xs_pred,
        ...
        # --- UHMC refinement (replaces ULA) ---
        # knobs: number of UHMC outer iterations (reuse n_ula), leapfrog steps L, integrator step size, damping gamma

        beta_t = extract(self.betas, next_noise_level, x_pred.shape)
        n_hmc = n_ula  # 2 HMC refinements per diffusion step
        hmc_L = 3  # 3 leapfrog substeps
        hmc_step = 0.1 * beta_t
        hmc_gam = 0.9  # partial momentum refresh
        mass = beta_t.sqrt()

        if n_hmc > 0 and next_noise_level[0, 5].item() > 20:
            with torch.no_grad():
                # Initialize momentum ~ N(0, M)
                if mass is None:
                    M_sqrt = torch.ones_like(x_pred)
                else:
                    # allow a scalar, channel-wise, or full-tensor mass
                    M_sqrt = (
                        mass if torch.is_tensor(mass) else torch.ones_like(x_pred) * float(mass)
                    )
                v = torch.randn_like(x_pred) * M_sqrt

                for _ in range(n_hmc):
                    # Partial momentum refresh (no accept/reject in UHMC)
                    if hmc_gam is not None and 0.0 <= hmc_gam < 1.0:
                        lam = torch.randn_like(v)
                        v = hmc_gam * v + (1.0 - hmc_gam**2) ** 0.5 * lam * M_sqrt

                    # L leapfrog steps using ∇U from model at k = next_noise_level
                    x_new, v_new = self._uhmc_leapfrog(
                        x_pred,
                        v,
                        next_noise_level,
                        external_cond,
                    ...
                    # Update state
                    x_pred, v = x_new, v_new

                    # Respect your mask (don’t change frames where curr == next)
                    mask = curr_noise_level == next_noise_level
                    x_pred = torch.where(self.add_shape_channels(mask), x, x_pred)

                    # Safety checks (same as your ULA block)
                    if torch.isnan(x_pred).any() or torch.isinf(x_pred).any():
                        print("UHMC produced nan/inf in x_pred")

        # if n_ula > 0 and next_noise_level[0, 5].item() > 20:
        #    for _ in range(n_ula):
    ...
    def _gradU_from_model(self, x, k, external_cond, external_cond_mask, **kwargs):
        """
        Compute ∇U(x) for UHMC from the model's predicted noise ε(x,k).
        From your ULA update: x <- x - β / sqrt(1-α_k) * ε + noise
        so ∇U(x) = ε / sqrt(1-α_k).
        """
        # k can be a tensor (B, T_frame?) like your other calls
        clipped_k = torch.clamp(k, min=0)

        # α_k
        alpha_k = self.alphas_cumprod[clipped_k]
        alpha_k = self.add_shape_channels(alpha_k)

        with torch.no_grad():
            model_pred_k, _ = self.model_predictions(
                x=x,
                k=clipped_k,
                external_cond=external_cond,
                external_cond_mask=external_cond_mask,
            ...
            eps = model_pred_k.pred_noise
            gradU = eps / (1.0 - alpha_k).sqrt()
        return gradU
    ...
    def _uhmc_leapfrog(
        ...
        **kwargs,
        ...
        """
        Unadjusted HMC leapfrog integrator.
        H(x,v) = U(x) + 0.5 * v^T M^{-1} v  with diagonal mass M (via mass_diag_sqrt).
        """
        ...
        gradU = self._gradU_from_model(x, k, external_cond, external_cond_mask, **kwargs)
        v = v - 0.5 * step_size * gradU

        for _ in range(L):
            # Full step in position
            x = x + step_size * (v / mass_diag)

            # Full step in momentum, except last iter we will do only half
            gradU = self._gradU_from_model(x, k, external_cond, external_cond_mask, **kwargs)
            v = v - (step_size * gradU)

        # Final half step correction
        v = v + 0.5 * step_size * gradU  # cancels the extra full step above

        return x, v

    def ddim_sample_step2(
        self,
        x: torch.Tensor,
        curr_noise_level: torch.Tensor,
        next_noise_level: torch.Tensor,
        ...
        # ---------- NEW: U-HMC controls ----------
        use_uhmc: bool = True,
        n_uhmc: int = 4,  # number of U-HMC updates at this noise level
        uhmc_leapfrog_steps: int = 4,  # L (per U-HMC update)
        uhmc_step_scale: float = 1.0,  # ε = uhmc_step_scale * β_t
        uhmc_damping: float = 0.9,  # γ in [0,1]; 0 = full refresh, 1 = keep momentum
        # -----------------------------------------
        **kwargs,
    ):
        """
        If use_uhmc=True, we apply unadjusted HMC at the fixed noise level 'next_noise_level'
        using leapfrog integration on the negative score (potential). Momentum partial refresh
        is controlled by 'uhmc_damping' (Algorithm 3 w/o accept/reject). See
        - ULA/HMC equivalence and score scaling: eps / sqrt(1 - alpha_bar_t).  [2302.11552, App. B.2]
        - HMC with partial momentum refresh & mass choice β_t:                   [2302.11552, App. B.1, B.3]
        - Do not clip intermediate x during MCMC:                                [2302.11552, App. B.4]
        """
        ...
        # DDIM-like proposal to the next noise level
        x_pred = x_start * alpha_next.sqrt() + pred_noise * c + sigma * noise

        # Only update frames where the noise level changes
        mask_same_level = curr_noise_level == next_noise_level
        x_pred = torch.where(self.add_shape_channels(mask_same_level), x, x_pred)

        # -------------------------
        # ULA refinement (existing)
        # -------------------------
        if (
            (not use_uhmc)
            and n_ula > 0
            and next_noise_level.numel() > 0
            and next_noise_level[0, 5].item() > 20
        ):
            for _ in range(n_ula):
                with torch.no_grad():
                    model_pred_ula, _ = self.model_predictions(
                    ...
                    # ULA step at fixed t (Eq. A8/A10): x <- x - β_t * score + √(2β_t) ξ
                    # Here score = eps / sqrt(1 - alpha_bar_t)
                    score = eps / ((1 - alpha_next).sqrt() + 1e-12)
                    x_pred = x_pred - beta_t * score + (2.0 * beta_t).sqrt() * eta

                    # respect mask
                    x_pred = torch.where(self.add_shape_channels(mask_same_level), x, x_pred)

                    if torch.isnan(x_pred).any() or torch.isinf(x_pred).any():
                        raise RuntimeError("NaN/Inf encountered in ULA update.")

        # -------------------------
        # NEW: U-HMC refinement
        # -------------------------
        if use_uhmc and n_uhmc > 0:
            with torch.no_grad():
                beta_t = extract(self.betas, next_noise_level, x_pred.shape)  # step scale & mass
                eps_h = uhmc_step_scale * beta_t  # ε = c * β_t
                mass = beta_t.clamp_min(1e-12)  # M = β_t I  (App. B.3)
                mass_sqrt = mass.sqrt()
                mass_inv = 1.0 / mass

                v = None  # carry momentum across U-HMC sub-steps if γ>0

                def neg_grad_logp(x_in: torch.Tensor) -> torch.Tensor:
                    # ∇U(x) = −∇log p_t(x); score = ∇log p_t(x) = eps / sqrt(1 - ᾱ_t)  (App. B.2)
                    mp, _ = self.model_predictions(
                        x=x_in,
                        k=next_noise_level,
                        external_cond=external_cond,
                        external_cond_mask=external_cond_mask,
                    ...
                    eps_pred = mp.pred_noise
                    score = eps_pred / ((1 - alpha_next).sqrt() + 1e-12)
                    return -score

                for _ in range(n_uhmc):
                    # Partial momentum refresh (Algorithm 3): v' = γ v + sqrt(1-γ^2) * N(0, M)
                    if v is None or uhmc_damping <= 0.0:
                        v = torch.randn_like(x_pred) * mass_sqrt
                    else:
                        lam = torch.randn_like(x_pred) * mass_sqrt
                        v = uhmc_damping * v + math.sqrt(1.0 - uhmc_damping**2) * lam

                    # Leapfrog integration (L steps), unadjusted (no MH accept/reject)
                    for _lf in range(uhmc_leapfrog_steps):
                        g = neg_grad_logp(x_pred)  # ∇U
                        v = v - 0.5 * eps_h * g  # half-step update on momentum
                        x_pred = x_pred + eps_h * (v * mass_inv)  # full-step update on position

                    # Momentum flip for time-reversibility with partial refresh convention
                    v = -v

                    # respect mask (keep positions where noise level unchanged)
                    x_pred = torch.where(self.add_shape_channels(mask_same_level), x, x_pred)

                    if torch.isnan(x_pred).any() or torch.isinf(x_pred).any():
                        raise RuntimeError("NaN/Inf encountered in U-HMC update.")
