...
from ..common.modules.utils import Module
from ..common.modules.utils import resolve_str_to_module as _resolve_str_to_module
    ...
    def __init__(
        ...
        for i_level in range(self.num_resolutions):
            ...
            if temporal_downsample[i_level]:
                down.time_downsample = resolve_str_to_module(temporal_downsample[i_level])(
                    block_in, block_in
                )
...
class Decoder(nn.Module):
    def __init__(
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if spatial_upsample[i_level]:
                ...
                if spatial_upsample[i_level] == "Spatial2xTime2x3DUpsample":
                    upsample_kwargs["is_first"] = i_level == len(range(self.num_resolutions)) - 1
        ...
        # ---- Out ----
        self.norm_out = Normalize(block_in)
        self.conv_out = resolve_str_to_module(conv_out)(block_in, 3, kernel_size=3, padding=1)
...
class VideoVAE(VAE):
        ...
        if self.is_causal:
            ...
            if temporal_length % self.temporal_pixel_length != 0:
                pad = repeat(
                    ...
                    "b c 1 h w -> b c t h w",
                    t=self.temporal_pixel_length - temporal_length % self.temporal_pixel_length,
                )
                x = torch.cat([pad, x], dim=2)
            x = rearrange(x, "b c (m t) h w -> (b m) c t h w", t=self.temporal_pixel_length)
        h = self._encode(x)
    ...
    def decode(self, z: torch.Tensor, desired_length: Optional[int] = None) -> torch.Tensor:
    ...
    def from_pretrained(cls, path: str, **kwargs) -> "VideoVAE":
        ...
        if len(checkpoint["optimizer_states"]) > 0 and "ema" in checkpoint["optimizer_states"][0]:
            state_dict = dict(
                zip(
