...
class UViT3D(BaseBackbone):
    ...
    def __init__(
        ...
        block_dropouts,  # list of floats
        num_updown_blocks,  # int
        num_mid_blocks,  # int
        num_heads,  # int
        pos_emb_type,  # str
        use_checkpointing,  # list of bools
        x_shape: torch.Size,
        max_tokens: int,
        ...
        **kwargs,
        ...
        self.use_checkpointing = list(use_checkpointing)
        ...
        self.block_dropouts = block_dropouts
        self.num_updown_blocks = num_updown_blocks
        ...
        block_type_to_cls = {
            ...
            "TransformerBlock": (
                partial(TransformerBlock, emb_dim=self.emb_dim, heads=num_heads)
                # if num_kv_tokens == 0
                # else partial(
                #    TransformerBlockKVMem,
        ...
        if num_kv_tokens > 0:
            # mem_block_indices = [i for i in range(self.num_mid_blocks // 3, 2 * self.num_mid_blocks // 3)]
            mem_block_indices = [i for i in range(self.num_mid_blocks)]
            # mem_block_indices = [i for i in range(2, self.num_mid_blocks // 2)]
            mem_block_cls = partial(
                TransformerBlockKVMem,
                emb_dim=self.emb_dim,
                heads=num_heads,
                num_kv_tokens=num_kv_tokens,
    ...
    @staticmethod
    def _checkpointed_forward(module: nn.Module, *args, use_checkpointing: bool = False) -> Tensor:
